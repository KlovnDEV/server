...
RegisterNetEvent("iconic-hud:changeRange")
AddEventHandler("iconic-hud:changeRange", function(pRange)
    voice = pRange or 2
end)
...
RegisterNetEvent("np-admin:currentDebug", function(pOn)
	DebugMode = pOn
end)

pShowLocation = false
RegisterNetEvent("np-inventory:watch:status")
AddEventHandler("np-inventory:watch:status", function()
      ...
      if IsControlJustReleased(0, 305) and GetLastInputMethod(0) then
        ...
        if beltOn then 
			TriggerEvent('seatbelt', true)
				TriggerServerEvent('InteractSound_SV:PlayOnOne', 2.0, 'seatbelt', 2.1)
		  isUiOpen = true 
		else 
			TriggerEvent('seatbelt', false)
				TriggerServerEvent('InteractSound_SV:PlayOnOne', 2.0, 'seatbeltoff', 2.1)
		  isUiOpen = true  
		end
      end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if isLoggedIn then
			...
			function backToVoiceHud()
				-- SendNUIMessage({action = "toggle_hud"})
				pRadio = false

			if pRadio == false then
...
RegisterNetEvent('iconic-gas:checkpump')
AddEventHandler('iconic-gas:checkpump', function()
...
AddEventHandler("fire:damageUser", function(Reqeuester)
	...
	if IsPedShooting(Attackerped) then
        ...
        if name == `WEAPON_FIREEXTINGUISHER` and not exports["iconic_handler"]:isPed("dead") then
		...
		if not IsPedSwimmingUnderWater( PlayerPedId() ) and currentValues["oxy"] < 25.0 then
			if GetGameTimer() - lastDamageTrigger > 3000 then
				...
				if currentValues["oxy"] <= 0 then
					...
					if exports["iconic_handler"]:isPed("dead") then
...
RegisterNetEvent("iconic-adminmenu:currentDevmode")
AddEventHandler("iconic-adminmenu:currentDevmode", function(devmode)
    isBlocked = devmode
end)
