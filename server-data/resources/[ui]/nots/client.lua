...
AddEventHandler('co_notify:client:SendNotifys', function(data)
    ...
    ESX.TriggerServerCallback('co:notify:check_phone', function(phone)
      telefon = phone
	  if telefon ~= nil then SendNUIMessage({ action = "bildirim", uygulama = data.app, baslik = data.title, icerik = data.content, juk = data.showing }) end
    end)
end)

function SendNotify(app, title, content, showing)
    if FirstRun == false then TriggerEvent('co_notify:first_run'); FirstRun = true  end
--[[    if app == "arama" then 
        Aramainfo = content
    ...
    if app == "youtube" then 
        SendNUIMessage({ action = "youtube", uygulama = app, baslik = title, icerik = content, juk = showing})
		Youtube = true
		DonguTrigger()
        return
    end;
	ESX.TriggerServerCallback('co:notify:check_phone', function(phone)
      telefon = phone
	  if telefon ~= nil  then SendNUIMessage({ action = "bildirim", uygulama = app, baslik = title, icerik = content, juk = showing}) end
	end)
end
