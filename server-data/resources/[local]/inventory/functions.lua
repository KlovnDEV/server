...
Citizen.CreateThread(function()
    -- exports["binds"]:registerKeyMapping("", "Vehicle", "Door Keyfob", "+useKeyFob", "-useKeyFob", "Y")
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if (itemid == "tuner") then
      local finished = exports["taskbar"]:taskBar(2000,"Connecting Tuner Laptop",false,false,playerVeh)
    ...
    if (itemid == "lsdtab" or itemid == "badlsdtab") then
        ...
        local finished = exports["taskbar"]:taskBar(3000,"Placing LSD Strip on 👅",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "decryptersess" or itemid == "decrypterfv2" or itemid == "decrypterenzo") then
      if (#(GetEntityCoords(player) - vector3( 1275.49, -1710.39, 54.78)) < 3.0) then
          local finished = exports["taskbar"]:taskBar(25000,"Decrypting Data",false,false,playerVeh)
          if (finished == 100) then
    ...
    if (itemid == "weedoz") then
      ...
      local finished = exports["taskbar"]:taskBar(5000,"Packing Q Bags",false,false,playerVeh)
        if (finished == 100) then
    ...
    if ( itemid == "smallbud" and hasEnoughOfItem("qualityscales",1,false) ) then
        local finished = exports["taskbar"]:taskBar(1000,"Packing Joint",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "weedq") then
        local finished = exports["taskbar"]:taskBar(1000,"Rolling Joints",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "lighter") then
        TriggerEvent("animation:PlayAnimation","lighter")
          local finished = exports["taskbar"]:taskBar(2000,"Starting Fire",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "joint" or itemid == "joint2") then
        local finished = exports["taskbar"]:taskBar(2000,"Smoking Joint",false,false,playerVeh)
        if (finished == 100) then
            Wait(200)
    ...
    if (itemid == "Gruppe6Card") then
        ...
        if targetVehicle ~= 0 and GetHashKey("stockade") == GetEntityModel(targetVehicle) and countpolice > 2 then
            ...
            if aDist < 2.0 then
                ...
                local finished = exports["taskbar"]:taskBar(timeToOpen,"Unlocking Vehicle",false,false,playerVeh)
                if finished == 100 then
                    remove = true
    ...
    if (itemid == "usbdevice") then
        local finished = exports["taskbar"]:taskBar(15000,"Scanning For Networks",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "heavyammo") then
        local finished = exports["taskbar"]:taskBar(5000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "pistolammo") then
        local finished = exports["taskbar"]:taskBar(2500,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "huntingammo") then
        local finished = exports["taskbar"]:taskBar(10000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "taserammo") then
        local finished = exports["taskbar"]:taskBar(2000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "armor") then
        local finished = exports["taskbar"]:taskBar(10000,"Armor",true,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "cbrownie" or itemid == "cgummies") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["taskbar"]:taskBar(3000,"Consuming edibles 😉",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "bodybag") then
        local finished = exports["taskbar"]:taskBar(10000,"Opening bag",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "bodygarbagebag") then
            local finished = exports["taskbar"]:taskBar(10000,"Opening trash bag",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "newaccountbox") then
        local finished = exports["taskbar"]:taskBar(5000,"Opening present",false,false,playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "foodsupplycrate") then
        ...
        local finished = exports["taskbar"]:taskBar(20000,"Opening the crate (ESC to Cancel)",false,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "fishingtacklebox") then
        local finished = exports["taskbar"]:taskBar(5000,"Opening",true,false,playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "organcooler") then
        local finished = exports["taskbar"]:taskBar(5000,"Opening cooler",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "Bankbox") then
        if (hasEnoughOfItem("locksystem",1,false)) then
            local finished = exports["taskbar"]:taskBar(10000,"Opening bank box.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Securebriefcase") then
        if (hasEnoughOfItem("Bankboxkey",1,false)) then
            local finished = exports["taskbar"]:taskBar(5000,"Opening briefcase.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Largesupplycrate") then
        if (hasEnoughOfItem("2227010557",1,false)) then
            local finished = exports["taskbar"]:taskBar(15000,"Opening supply crate.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "xmasgiftcoal") then
        local finished = exports["taskbar"]:taskBar(15000, "Opening Gift", false, false, playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
            ...
            Citizen.CreateThread(function()
                while finished ~= 100 and not cancelNos do
                    ...
                    if GetEntitySpeed(GetVehiclePedIsIn(player, false)) > 11 then
                        exports["taskbar"]:closeGuiFail()
                        cancelNos = true
                    end
                end
            end)
            finished = exports["taskbar"]:taskBar(20000,"Nitrous")
    ...
    if (itemid == "ciggy") then
        local finished = exports["taskbar"]:taskBar(1000,"Lighting Up",false,false,playerVeh)
        if (finished == 100) then
            Wait(300)
    ...
    if (itemid == "oxygentank") then
        local finished = exports["taskbar"]:taskBar(30000,"Oxygen Tank",true,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "sandwich" or itemid == "hamburger") then
        ...
        TriggerEvent('esx_status:add', 'hunger', 130000)
    end
    ...
    if (itemid == "cola" or itemid == "water") then
        ...
        TriggerEvent('esx_status:add', 'thirst', 130000)
    end
    ...
    if (itemid == "methlabproduct") then
        local finished = exports["taskbarskill"]:taskBar(1000, math.random(10, 15))
        if finished == 100 then
            TriggerEvent("attachItemObjectnoanim","crackpipe01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if (itemid == "shitlockpick") then
        ...
        local finished = exports["taskbarskill"]:taskBar(1000, math.random(5, 15))
        if (finished == 100) then
    ...
    if (itemid == "harness") then
        ...
        finished = exports["taskbar"]:taskBar(20000,"Harness")
...
function TaskItem(dictionary,animation,typeAnim,timer,message,func,remove,itemid,playerVeh,itemreturn,itemreturnid,quality)
    ...
    if timer > 0 then
        local finished = exports["taskbar"]:taskBar(timer,message,true,false,playerVeh)
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            local finished = exports["taskbarskill"]:taskBar(15000, 3)
            ...
            local finished = exports["taskbarskill"]:taskBar(5000,3)
            ...
            local finished = exports["taskbarskill"]:taskBar(2000, 8)
...
-- RegisterNetEvent("useitem:eat:maikavi")
